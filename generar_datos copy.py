...
with app.app_context():
                ...
                if response_tmp.status_code != 200:
                    ...
                    for r in response:
                        ...
                        for p in r['products']:
                            ...
                            row = [
                                x.store_id, r['number'], r['created_at'],r['status'], p['id'], p['quantity'], r['subtotal'], r['discount'], 
    ...
    with open('logs/app/datos_productos.csv', 'w+', newline='') as p:
        writer = csv.writer(p)
        header = ['Tienda', 'Nro_orden', 'Fecha_orden', 'Status','Producto','Cantidad', 'Subtotal', 'Descuento', 'Shipping', 'Cliente', 'Provincia', 'Localidad', 'Codigo_Postal']
        writer.writerow(header)
